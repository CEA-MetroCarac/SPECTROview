...
class Maps(QObject):
    ...
    def __init__(self, settings, ui, spectrums, common, visu):
        ...
        self.progressChanged.connect(self.update_pbar)
    ...
    def apply_loaded_fit_model(self, fnames=None):
        ...
        args = (
            self.loaded_fit_model, fnames, ncpus, fit_only, show_progressbar)
        thread = Thread(target=self.spectrums.apply_model, args=args)
        thread.start()

        while self.spectrums.pbar_index < len(fnames):
            percent = 100 * (self.spectrums.pbar_index + 1) / ntot
    ...
    def update_pbar(self, percent):
        ...
        """
        print(f"Updating progress bar: {percent}%")  # Debugging line
        self.ui.progressBar.setValue(percent)

    def update_ptext(self, text):
        """
        Update the progress text in GUI.
        """
        print(f"Updating progress text: {text}")  # Debugging line
        self.ui.progress.setText(text)

    def fit_completed(self):
        """
        Actions to perform upon completion of the fitting process.
        """
        self.upd_spectra_list()
